...
| **音效（合成）** | ✅ 完成 | 第二十二轮：`src/audio/sfx.js` + `SoundToggle.jsx`。四个音全部 Web Audio **现场合成**（蓄势嗡 / 释放啪 / 翻牌唰 / 揭晓叮），**零素材零依赖**；时刻全部取自 `drawBeats` 那张节拍表。**默认关**，ctx 只在用户手势里建。开关在左上角（品牌下方）—— 不进顶栏（那是窄屏标题字号的预算）、不放右下角（会被解读面板盖住） |
...
- **🆕 中文衬线子集字体（第二十一轮列为「美感上最大的一处欠账」）**：
  全站目前 **`@font-face` 数量为 0**，标题/牌名/低语全部靠系统字体栈兜底 ——
  同一份页面在 Windows / macOS / Android 上字形差别很大，而这是个**以氛围为卖点**的站点。
  做法：挑一款开源中文衬线（思源宋体 / 霞鹜文楷一类）→ **按站点实际用到的字做子集**（`fonttools pyftsubset`，
  统计口径取 `src/data/cards.js` + `whispers.js` + `App.jsx` 里的全部字面量，去重后通常只有 600–900 字）
  → 子集后 woff2 约 **150–400 KB**，`font-display: swap` + 只给 `--font-display` 用（正文继续留系统字体）。
  ⚠️ 别整包里放全套字库（思源宋体全集 15 MB+，首屏直接崩）。⚠️ 字重只留 1–2 档。
- **✅ 已做（第二十二轮）· Web Audio 合成音效**：见 `src/audio/sfx.js`。蓄势 / 释放 / 翻牌 / 揭晓四个音，
  全部现场合成（零素材、零依赖），默认关，开关在左上角。**后续可扩的方向**（都还没做）：
  · 图鉴翻页 / 详情打开的轻音（现在只有抽牌这条线有声音）；
  · 音量滑杆（现在是固定 `MASTER_GAIN = 0.34`）；
  · 环境底噪（很轻的雾声循环）—— 但这会变成「一直响」，要先想清楚怎么和静音礼仪共存。
  ⚠️ 改动任何音频代码前先读 `sfx.js` 头部那三条规矩，并跑 `probe-sfx` + `probe-sfx-off`。
...
**第二十一轮新增的三个探针**（都在 `scripts/flows/`，用 `run-flows.mjs` 调）：
...
| `probe-panel-worst` | **逐张遍历 22 张牌**，量每张的面板净空与重叠，报最坏 3 张 | `audit-draw` 只抽**随机一张**，抽不到文案最长的那张 → 拿不到最坏情况。这里靠写 `localStorage` 记录 + 切开发模式把 22 张**确定性地**过一遍 |
...
"$N" scripts/run-flows.mjs probe-panel-worst --reduced       # 最坏净空（门槛 8px）
"$N" scripts/run-flows.mjs probe-sfx probe-sfx-off           # 音效：开了会响 / 没开不建 ctx
"$N" scripts/run-flows.mjs audit-title --w 1582 --h 700      # 矮视口：标题不得被顶出屏幕上缘
```
