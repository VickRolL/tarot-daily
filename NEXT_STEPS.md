...
## 28 · 2026-09-22 第三十七轮：把「已上线」同步到 GitHub 仓库层面
...
## 29 · 2026-09-22 第三十七轮续：仓库转为公开（含转公开前的全历史审计）

用户要求把 `VickRolL/tarot-daily` 从私密改成公开给朋友看。**已转 PUBLIC。**

### 先分清两件事（用户最容易误解）

1. **网站本来就是公网地址** —— 朋友现在就能看，**公开仓库不是「给人看」的必要条件**。
   「给朋友看」要先问清给的是**网址还是代码**。
2. **公开仓库 ≠ 自动部署** —— 仍是手工部署站点（改代码 `push` 后线上不会变，见 §27.2）。

### 转公开前的审计：`scripts/_scan_history_secrets.py`

必须扫**整条历史**，不能只扫当前快照 —— 一个曾提交、后来删掉的密钥，
在当前 checkout 里看不到，在 GitHub 页面上翻两下就有，而且**永久留在历史里**（除非改写历史）。

结果（**692 个对象 / 327 个文本 blob**）：

- **密钥零命中**；`.env.local`（Netlify token + ElevenLabs key）**从未入库**；
- 无 `.env` / `*.pem` / `*.key` 类路径；无 >50MB 文件（最大 2.94 MB，GitHub 单文件上限 100 MB）；
- 转公开时点 **0 fork / 0 star** —— 从没被别人复制过，**这是动手的最佳时机**。

**三条扫描器本身的纪律**（都踩过）：

- **只扫文件内容会漏掉提交作者邮箱** —— 它在 commit object 里，必须单独查
  `git log --format='%an <%ae>'`；
- **个人标识不会触发密钥规则** —— 手机号嵌在句子里长得完全不像凭据，
  扫描器**必须显式带手机号 / 邮箱规则**，否则给出「干净」的**假结论**；
- 误报自己滤：扫出的「QQ 号」命中全是假的（`12000`/`44100` 是音频采样率、`20260919` 是日期）。

### ⚠️ 已随公开暴露、且**用户知情后选择接受**的内容

- `NEXT_STEPS.md` 里作为 Netlify 账号名写着的一段**手机号**（**4 个提交**均含）；
- **提交作者邮箱** `2992308088@qq.com`（**38 个提交 + 4 个标签**全带）。

想反悔就得**改写历史**（`filter-branch` / `filter-repo` + force push）——
代价是**全部 commit SHA 与 v4 标签都变**；线上站不受影响（部署的是本地 `dist/`）。
**只改 HEAD 上的文件没用**，旧版本仍在历史里。

> **本节写法本身也是一条纪律**：新增文档**不要再重复那段手机号**，
> 用「Netlify 账号名里嵌的手机号」指代即可 —— 已经暴露了，就不该继续增加暴露面。

### 验证：必须用**匿名视角**

本机 `gh` 是带认证的 —— **认证视角说 public ≠ 匿名访客真能打开**。
用**不带任何凭据**的请求复验：仓库 API 200（`private:false`）、仓库网页 200、
raw README 200（首行正确）、Release 页 200、随机抽 6 个真实素材全 200。

> 顺带又被自己骗一次：随手猜 `public/cards/major-00.webp` 得 **404**，
> 真实路径是 `public/skins/mist-night/cards/...`。**「猜路径」也是一种判据错。**
