...
- **🆕 Web Audio 合成音效（零素材成本，第二十一轮提出）**：充能 / 翻牌 / 揭晓三个音。
  用 `OscillatorNode` 现场合成（低频正弦 + 噪声包络 = 「嗡」与「咔」），**不引入任何音频素材**、不加依赖。
  必须满足：① **默认静音**，由用户点一个开关打开（自动播放策略 + 礼仪）；
  ② `AudioContext` 必须在用户手势里创建/`resume`；③ `prefers-reduced-motion` 时不给动画配「必有音」；
  ④ 抽完牌后不重复播放（每日一签只响一次）。⚠️ 别用 `<audio>` 标签加载 mp3 —— 那会破「零素材」这条线。
