...
export const ANCHORS = {
  /**
   * 2026-09-19 第八轮：size 31 → 40。
   *
   * 起因是用户拿自己的参考图否掉了第 1 版主视觉，其中一条是「球太小、不够主角」。
   * 只动了 size 一个数 —— **x / y 一动都不动**，于是：
   *   · 球心仍在 (50%, 61.5%)，`CARD_RISE`（锚的是卡牌落点 38.5%，不是球心）不用重算
   *   · 卡牌面板契约（overlapPx = 0 / 净空 ~39px）与标题带全部不受影响
   * 代价（实测确认过才准写死）：1564×708 下球的视口 rect 由 336px 变 434px，
   * 球顶由 386px 升到 337px，距卡牌底边（272.6px）仍有 64px 净空；球的下缘
   * 由超出视口 14px 变成超出 63px，被解读面板盖住的比例由 73.5% 降到约 67%。
   * 手机上球宽 351px、而可见窗口横带约 395px，左右各剩约 22px 余量 —— 很挤但成立。
   */
  orb: { x: 50, y: 61.5, size: 40 },
  /**
   * 牌面落点（视口坐标系）。
   *
   * 2026-09-18 手感升级：从 44 上移到 38.5。原来卡牌 48vh 占 20%→68%，
   * 而解读面板顶边在 66.4% —— **面板会压住卡牌 12.7px**（实测），牌面下框带被啃掉一条。
   * 卡高收到 46vh、落点上移到 38.5，牌占约 15.5%→61.5%，
   * 与面板顶边（66.4%）留出约 39px 净空，彻底不压。
   *
   * 2026-09-20（第二十三轮）：38.5 → 40，同时卡高 46vh → 43vh（下半 23 → 21.5）。
   * 用户反馈「**卡牌有点靠上，与副标题有点重叠**」。
   *
   * ⚠️ 这里有个容易踩空的地方：**标题带是从卡牌顶边往上锚定的**
   *    （`.headline { bottom: 100% − anchorY + halfH + gap }`），所以
   *    「只把牌往下挪」根本没用 —— 标题带会**跟着一起往下挪**，
   *    副标题与牌面之间那 20px 纹丝不动，改了等于没改。
   *    要真正拉开距离，只能**同时把 TITLE_GAP 加大**。
   *
   * 但加大 gap 会把标题带整块往上顶，而 @612 高视口下标题带顶端只剩 14px 余量。
   * 于是采用「**下移换空间**」：
   *     · 落点 38.5 → 40、下半高 23 → 21.5  →  牌面顶边 15.5% → 18.5%（+3%）
   *     · TITLE_GAP 20 → 38                                      （−18px）
   *     · 标题带绝对位置净变化 = +3%h − 18px ≈ **+0.4px(@612) / +3.2px(@708)**
   *   即：**牌往下走了 18~24px，标题几乎没动，副标题与牌之间的净空翻倍到 38px。**
   *
   * 而且底边 40 + 21.5 = **61.5%，与改前完全相同** —— 面板净空一点没损失
   *  （这是刻意的：竖屏下那个净空只剩 10.1px，扣不起）。
   */
  stage: { x: 50, y: 40 }
}
...
/* ==========================================================================
   卡牌与标题的几何契约（2026-09-18 修「标题被卡牌盖住」，2026-09-20 调间距）
   --------------------------------------------------------------------------
   标题带（「今夜一签」+ 副标题）**从卡牌顶边往上锚定**，不再从视口顶往下排。
   ...
   ⚠️ **TITLE_GAP 不是「随手加的留白」，它是副标题与牌面之间唯一的可调量。**
      挪卡牌、改卡高都只是让两边一起平移，间距恒等于 gap。想改间距只能改它，
      而改它会把标题带整块往上顶（可用余量见 index.css 里 .headline__title 的公式）。

   ⚠️ 若改 CARD_HEIGHT，务必同步改 index.css 里 .card 的 height 字面量。
   ========================================================================== */
export const CARD_HEIGHT = 'min(43vh, 420px)'
export const CARD_HEIGHT_HALF = 'min(21.5vh, 210px)'
export const TITLE_GAP = '38px'

/* ==========================================================================
   两侧低语标签（2026-09-20 第二十一轮）
   --------------------------------------------------------------------------
   起因：主页左右两侧是空的，「巫师正在对你说话」这件事只能靠水晶球下面
   那行提示文字承担。这里用一组缓慢轮切的短语把两侧填上，同时把这层含义说出来。

   ⚠️ 坐标是**视口坐标系**（同 ANCHORS.stage），不是主视觉图像坐标系。
   ⚠️ `x` 的含义是「离最近那条边的百分比」——左侧即 `left`，右侧即 `right`。
      所以两侧写同样的数值就是镜像，不必手算 100−x。
   ⚠️ **安全区**：中间一列必须留给标题（y 4%~13%）与牌面
      （桌面实测 x 42.8%~57.2%，竖屏更宽）。所以左侧 x + 文本宽度、
      右侧同理，都不能侵入 30%~70% 这条带。
      改了 x / y 一定要重跑 `scripts/flows/probe-whispers.js` 核净空。
   ========================================================================== */
export const WHISPERS = {
  enabled: true,
  /**
   * 每侧 5 条。x = 离本侧边缘的百分比，y = 离顶部的百分比。
   *
   * ⚠️ **2026-09-20 字号从 11–14px 提到 13–18px，这四个「缩进位」跟着往回收了**
   * （12/14 → 10/11.5）。原因是 x 与字号是乘起来的：文字越宽，越往中间顶。
   * 两条都往中间靠：
   *   · 左侧 x 大 → 内边缘（右边缘）越容易越过 34% 警戒线；
   *   · 右侧 x 大 → 内边缘（左边缘）越容易跌破 66%。
   * 实测最险的是 reduced 静态子集里那条 16 字长句（「所有问题都有答案，只是有的要等」），
   * 旧 x=14 + 新字号算出来是 33.5% —— 只剩 0.5% 余量，等于没有。
   * 缩到 11.5 之后约 30.6%，留出 3.4%（约 53px）。
   * ⚠️ 改这里必跑 `scripts/flows/probe-whispers.js`，它逐条量 innerPct。
   */
  layout: [
    { side: 'left', x: 4.5, y: 19 },
    { side: 'left', x: 10, y: 33 },
    { side: 'left', x: 5.5, y: 47 },
    { side: 'left', x: 11.5, y: 61 },
    { side: 'left', x: 6.5, y: 75 },
    { side: 'right', x: 4.5, y: 15 },
    { side: 'right', x: 10, y: 29 },
    { side: 'right', x: 5.5, y: 43 },
    { side: 'right', x: 11.5, y: 57 },
    { side: 'right', x: 6.5, y: 71 }
  ],
  /**
   * 一条低语「亮着」与「暗着」的时长（毫秒）。
   *
   * ⚠️ 这两条决定了「同一时刻看得见几条」：占空比 = hold/(hold+gap)。
   * 2026-09-20 调过两轮，都以截图为准：
   *   5200/4200 = 55% → 每侧只剩 2 条亮着，两侧又读回「空」了，等于没做
   *   6600/3200 = 67% → 每侧约 3 条同时在场（当前值）
   * 调到 80% 以上会变成一整片静止的字（「轮切」这件事就没了）；低于 50% 退回第一版。
   *
   * ⚠️ **相位是「把一整个周期在同侧各条之间均分」，没有单独的 stagger 参数。**
   * 踩过：第一版按「第 r 条延后 r×1.7s」错开，看起来合理，实际是**齐涨齐落** ——
   * 因为 4×1.7=6.8s 远小于周期 9.4s，所有「亮起」窗口都挤在周期开头，
   * 于是页面在「10 条全亮」和「只剩 1 条」之间来回摆。实测采样到 10/6/2 就是这个毛病。
   * 均分之后任意时刻稳定在 3~4 条，且开页时 4/5 已经亮着（不必等 8 秒才铺满）。
   * 详见 WhisperTags.jsx 里那段 `offset` 的推导。
   */
  holdMs: 6600,
  gapMs: 3200,
  /** 指针视差的位移上限（px），乘各自的深度系数。纯氛围，别调大 */
  parallax: 14
}
