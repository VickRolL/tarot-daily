...
⚠️ **仓库已转为 PUBLIC**（第三十七轮续，用户要求「给朋友看看」）：转公开前扫过**整条历史**
（692 对象 / 327 文本 blob）—— **无密钥**，`.env.local`（Netlify token + ElevenLabs key）
**从未入库**；转公开时点 0 fork / 0 star。但有两处**个人标识随公开暴露、且用户知情后选择接受**：
`NEXT_STEPS.md` 里作为 Netlify 账号名写着的一段手机号，以及**作者邮箱**（38 个提交 + 4 个标签全带，
在 commit 元数据里 —— **文件内容扫描扫不到**）。想反悔须**改写历史**（会换掉全部 commit SHA
与 v4 标签；线上站不受影响），**只改 HEAD 上的文件没用**。详见 `NEXT_STEPS.md` §29。
⚠️ 注意区分：**网站本来就公开**（朋友一直能看）≠ **仓库公开**；且仓库公开**不会**让站点变成自动部署。
...
`origin/main`（远端此前停在 R29 `3f40239`，fast-forward `3f40239..2a37b36`），并在同一提交上打
**附注标签 `v3`** 一起推送；标签注释里写清这一版面向用户的改动与「未做本地冻结快照」。
⚠️ 密钥扫描这次是**推完之后**才补做的（顺序反了 —— R29 那次是先查后推，这次漏了），
所幸 `3f40239..v3` 的 diff 扫下来零命中（29 个文件 / 2796 新增行 / 9 类模式）。**下次必须先扫后推。**
⚠️ **本轮没有做 v1/v2 那种本地冻结快照**（`_archive/` 不进版本库，本地已有两份），需要可按 §20 流程补。
详见 `NEXT_STEPS.md` §23。上一轮第三十二轮：**每张牌 3~5 条「今日建议」+ 图鉴里查不到** ——
...
5. ~~⚠️ **上线前必补：favicon**~~ → **✅ 已做（第三十四轮）**：`scripts/build_favicon.py`
   从牌背 `card-back.webp` 自动裁出 `favicon.ico`(16/32/48) + `icon.png`(192) +
   `apple-touch-icon.png`(180)，`index.html` 已补三条 `<link rel="icon">`。做法见 `NEXT_STEPS.md` §2 P1
...
### 第三十三轮 · 发到 GitHub（`v3` 标签）（2026-09-21）

本轮**没有改功能代码**。用户验完开发者版说「很好，打包好更新到 github 吧」，
于是做的是「归档 + 同步远端」：让 GitHub 上留下一个能回指的版本点。

| 动作 | 结果 |
|---|---|
| `git push origin main` | `3f40239..2a37b36`（fast-forward）。推送前远端 `main` **正好等于 `v2`** |
| `git tag -a v3` + `git push origin v3` | 远端出现 `[new tag] v3 -> v3`；`v3^{}` = `2a37b36` = `main` |
| 生产构建 | `vite build` 干净通过（423 modules；只有 `three.module` 746.94 kB 的体积警告，一贯如此） |
| 密钥扫描（**推完才补做**） | 29 个文件 / 2796 新增行；`sk-` / `AIza` / `ghp_` / `AKIA` / `Bearer` / `apiKey=` / `token=` / `BEGIN … PRIVATE KEY` **全 0**；`.env.local` 唯一命中是文档里的一句描述 |

**为什么标签在这一版特别重要**

`dist/` 与 `_archive/` 都在 `.gitignore` 里 —— GitHub 上既没有构建产物、也没有冻结快照。
「GitHub 上的哪个状态算哪一版」只能靠标签回答。`v3` 的注释按 v1/v2 的体例写了本版要点
（声音默认开 + 喇叭开关 / 每张牌 3~5 条建议 / 探针变化），并**显式写明「本次未做本地冻结快照」**：
v1、v2 都有 `_archive/` 下对应的目录，v3 没有 —— 不写出来，接手的人会去目录里翻第三份。

**两条与「推送」有关、下次照做的前提**

1. **密钥扫描必须在 push 之前做**（推上去等于公开、不可撤销）。R29 记过这条，本轮我自己漏了、
   是推完才补扫的 —— **零命中是运气好，不是流程对**。
2. **本机不能用 `git branch -vv` 判断「远端有没有」**。本轮专门复查过：
   `remote.origin.fetch` = `+refs/heads/*:refs/remotes/origin/*`、`branch.main.{remote,merge}`
   都**配置正确**，但 `git for-each-ref refs/remotes` **一条都没有** —— 是环境丢弃引用写入
   （第 6 节第 33 条），不是配置问题。所以 `[origin/main: gone]` 在本机是**常态**，
   与「推送成功没有」无关；判据只能用 `git ls-remote origin`（本轮靠它确认 `main = 2a37b36`）。
   另：仓库是 **private**（匿名访问 `api.github.com/repos/...` 返回 404 反证）。

### 第三十四轮 · 首次上线（EdgeOne Makers）+ 补 favicon（2026-09-22）

用户：「接下来要做的就是尝试部署网站了」。站点**第一次有了公网地址**，同时把「上线前必补」的 favicon 补掉。

**① 通道选择：EdgeOne Makers（腾讯云）**

先摆事实再让用户拍板：`dist/` 实测 **7.2 MB / 39 文件**、纯静态零后端 → 任何静态托管都吃得下；
仓库是 **private** → **GitHub Pages 免费版用不了**（要 Pro），但 Vercel / Cloudflare / 腾讯云都能连私有仓。
用户选了 **EdgeOne Makers**（此后又提到他配好了 CloudBase 环境，但那条在我这边需要额外授权连接器才能驱动，
且当前已按选择完成部署，故未改道）。

部署命令与硬约束见 `NEXT_STEPS.md` §2 P1（该节在第三十五轮由三条增补为四条，新增的一条见 §25.1）。
**验收用真浏览器做的**：
`readyState=complete`、`hasCanvas=true`（WebGL 球起来了）、三个图标 link 全部生效、中文正文完整。

**② favicon：补上了，但脚本第一版是错的**

新增 `scripts/build_favicon.py` —— 从牌背 `card-back.webp` **自动探测深绿内框**再裁正方形，
产出 `favicon.ico`(16/32/48) + `icon.png`(192) + `apple-touch-icon.png`(180)。

第一版跑出来「看着像成功了」，实际是错的：脚本报的深绿内框是 **594×879（几乎整张图）**，
裁切框甚至超出画布（x 到 639 > 画布宽 620），产出里**星形偏左、右侧多一条米色竖条**。
根因：牌背**最右一列与最下一行有一道 1px 的暗色描边**，用「全图暗像素 bbox」取内框会被它一撑到底，
**而且脚本不报任何错**。加 `EDGE_SKIP=6` 排除最外圈后内框才是真的 568×851。

选材与取值也各淘汰过一个方案（都是**看图**淘汰的，不是算出来的）：
· 星云 `orb-nebula.webp` 是无形状纹理 → 16px 下认不出；
· 「整张牌背 contain 进正方形」→ 要填色，而填充色若取样到内框中心的星形上会变成**米色底**，
  在浅色标签栏上几乎看不见（第一版真踩到了，`fill=(204,188,162)`）；
· 边长取内框高 → 超过画布宽，会把左右米色竖条带进来。
最终 **边长 = 内框宽**：既有几何含义（不是手调魔数），水平又恰好贴合深绿区。
主图标定 **192 而非 512**：512 的 PNG 实测 **543 KB**，而标签页真正下载的是 ico 里的 16/32/48，
192 已覆盖书签栏/任务栏最大显示尺寸，体积降到 **75 KB**。

**③ 顺手堵住的一个坑**

`edgeone makers deploy` 会在仓库根留下 **`.edgeone/`（44 个文件 = dist 副本）**，而它**不在 `.gitignore` 里** ——
`git status` 里是未跟踪状态，`git add .` 会把 44 个构建产物卷进仓库。已加入忽略并注明理由。

**④ 明确「不改」的**：`og:image` / `og:url` / `twitter:image` 保持相对路径。
当前预览链接带鉴权，抓取端过不了 token 校验，**改了也不生效** → 等绑正式域名时一次改对。
（顺带核出：要改的是**三**处不是两处，`twitter:image` 在旧文档里一直被漏掉。）

---

### 第三十五轮 · 查清「给别人看」还差什么（2026-09-22）

用户：「目前还差什么才能给别人看到我的网站？」本轮**没改功能代码**，只做核查与实测。
答案全在**平台规则**里 —— 方向猜错会白做（比如去改 og 标签、或去绑 CloudBase 默认域名）。

**① 查出一条之前没记的硬约束：预览链接只有 3 小时有效期**

EdgeOne 官方文档写明：通过「项目域名」与「部署域名」访问，**必须走系统生成的预览链接，
有效期为 3 小时，超时返回 401**。这条之前只被记成「必须带 token」，差别却是致命的 ——
旧认知是「链接永久有效、只是长了点，可以随手发人」，事实是**发出去第二天打开就是 401**。

顺带验明 `eo_time` 的语义：**它是签发时刻，不是截止时刻**。判据是自洽性 ——
旧链接 `eo_time=1790011250`（01:20:50 签发）在 01:32 仍能打开；若它是截止时刻，当时就该失效了。

诚实标注：「3 小时」是**官方规定值，本轮未实测到期那一刻**（总不能干等 3 小时）；
实测到的只有「签发后 12 分钟仍可用」。

**② 续期机制实测：重新部署 = 新链接（54 秒）**

`makers deploy -n tarot-daily --json` 重跑一次，`eo_token` / `eo_time` / `deploymentId` **三者全部换新**，
本次 `eo_time` 比上次晚 **797 秒**（正好等于两次部署的间隔 → 再次印证它是签发时刻）。
所以「过期了怎么办」的答案是**重新部署一次**，不必等控制台（控制台要用户本人登录，CLI 这条路我能代做）。

**③ 结论：没有三全方案**

「立刻可用 / 国内能直接打开 / 地址长期不变」三项，三条路各缺一项：

| 路径 | 立刻 | 国内直开 | 长期不变 |
|---|---|---|---|
| 继续发预览链接 | ✅ | ✅ | ❌ 3 小时 |
| Vercel / Cloudflare Pages | ✅ | ⚠️ 时通时不通 | ✅ |
| 域名 + ICP 备案 + 绑 EdgeOne | ❌ 约 7～20 天 | ✅ | ✅ |

**CloudBase 也不能当方案**：它的默认域名 `*.tcloudbaseapp.com` 会弹一道「访问提示中间页」，
访客必须先点「确定访问」；官方明确写「仅建议用于开发测试，**严禁用于正式生产环境或分发给大规模用户**」
→ 发给 HR 或客户，第一眼是风险提示页，比预览链接还差。
...
- **2026-09-22（第三十七轮续 · 仓库转为公开 + 转公开前的全历史审计）**
  - 用户要求把仓库从私密改成公开「给朋友看看」。**已转 PUBLIC**。
    先讲清两件事：① **网站本来就是公网地址**，朋友一直能看，公开仓库不是必要条件；
    ② 公开仓库**不等于**自动部署（仍是手工部署站点）。
  - **新增 `scripts/_scan_history_secrets.py`**：扫**整条历史**（不是当前快照）——
    曾提交、后来删掉的密钥在当前 checkout 里看不到，在 GitHub 上翻两下就有。
    结果 **692 对象 / 327 文本 blob：密钥零命中**；`.env.local` 从未入库；
    无 `.env` / `*.pem` / `*.key` 路径；无 >50MB 文件（最大 2.94 MB）；
    转公开时点 **0 fork / 0 star**（= 从没被别人复制过，动手的最佳时机）。
  - **查出两处个人标识，用户知情后选择「接受现状直接公开」**：`NEXT_STEPS.md` 里
    作为 Netlify 账号名写着的一段手机号（4 个提交均含）、作者邮箱（38 提交 + 4 标签全带）。
    ⚠️ 作者邮箱在 **commit 元数据**里，**文件内容扫描扫不到**，必须 `git log --format='%an <%ae>'`
    单独查 —— 这是本轮新学到的（旧扫描器会给出「干净」的假结论）。
  - **验证必须用匿名视角**：本机 `gh` 带认证，认证视角说 public ≠ 匿名访客真能打开。
    用不带凭据的请求复验：仓库 API / 网页 / raw README / Release 页全部 200，
    随机 6 个真实素材也 200。（顺手被自己骗一次：猜的素材路径 404，真实路径在
    `public/skins/mist-night/cards/` 下 —— **「猜路径」也是一种判据错**。）
  - **技能整理**：新建独立技能 **`repo-public-release-audit`**（含扫描脚本资产）——
    这套审计适用于**任何**仓库，藏在部署技能里用户不会想到去翻；同时把
    `netlify-static-deploy` 里那一节收敛成指针，避免两处漂移。详见 `NEXT_STEPS.md` §29。
