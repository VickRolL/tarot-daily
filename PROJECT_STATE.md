...
最后更新：2026-09-22（第三十四轮：**首次上线 —— 部署到腾讯云 EdgeOne Makers**，并补齐 favicon ——
静态站已能从公网访问（项目 `tarot-daily`，global 区，`projectId` `makers-knnrulr5duup`）。
⚠️ 预览链接**必须带完整 `?eo_token=...&eo_time=...`**，去掉即 401；用 curl / urllib 之类纯 HTTP
客户端访问同样 401 —— 网关要求浏览器 JS 校验 token，**这是网关设计，不是故障**。
⚠️ `og:image` / `og:url` / `twitter:image` **仍是相对路径**：预览链接带鉴权，社交平台抓取端过不了
token 校验，**改了也不生效**，等绑正式域名再一次改对（注意是**三**处，现存文档只记了两处）。
详见 `NEXT_STEPS.md` §24。上一轮第三十三轮：**发到 GitHub（`v3`）** —— 把本地 R31 / R32 两个提交推到
...
部署命令与三条硬约束见 `NEXT_STEPS.md` §2 P1。**验收用真浏览器做的**：
